...
from aitutor.pydantic_settings_file import *

# aitutor 数据目录，必须通过环境变量设置。如未设置则自动使用当前目录。
AITUTOR_ROOT = Path(os.environ.get("AITUTOR_ROOT", ".")).resolve()
...
class BasicSettings(BaseFileSettings):
    ...
    model_config = SettingsConfigDict(yaml_file=AITUTOR_ROOT / "basic_settings.yaml")
    ...
    KB_ROOT_PATH: str = str(AITUTOR_ROOT / "data/knowledge_base")
    """知识库默认存储路径"""

    DB_ROOT_PATH: str = str(AITUTOR_ROOT / "data/knowledge_base/info.db")
    """数据库默认存储路径。如果使用sqlite，可以直接修改DB_ROOT_PATH；如果使用其它数据库，请直接修改SQLALCHEMY_DATABASE_URI。"""

    SQLALCHEMY_DATABASE_URI: str = "mysql+pymysql://root:openhydra@aes-ai-tutor-instances.mysql-operator.svc:3306/aes-ai-tutor"
    """知识库信息数据库连接URI"""
...
class KBSettings(BaseFileSettings):
    ...
    model_config = SettingsConfigDict(yaml_file=AITUTOR_ROOT / "kb_settings.yaml")
    ...
    DEFAULT_SEARCH_ENGINE: t.Literal[
        "bing", "duckduckgo", "metaphor", "searx"
    ] = "duckduckgo"
    """默认搜索引擎"""
    ...
    KB_INFO: t.Dict[str, str] = {
        "samples": "关于本项目issue的解答"
    }  # TODO: 都存在数据库了，这个配置项还有必要吗？
...
class PlatformConfig(MyBaseModel):
    ...
    platform_type: t.Literal[
        "xinference", "ollama", "oneapi", "fastchat", "openai", "custom_openai"
    ] = "xinference"
    """平台类型"""

    api_base_url: str = "http://service-supervisor.xinference.svc:9997/v1"
    """openai api url"""
...
class ApiModelSettings(BaseFileSettings):
    ...
    model_config = SettingsConfigDict(yaml_file=AITUTOR_ROOT / "model_settings.yaml")
    ...
    MODEL_PLATFORMS: t.List[PlatformConfig] = [
        PlatformConfig(
            **{
                ...
                "api_base_url": "http://service-supervisor.xinference.svc:9997/v1",
                "api_key": "EMPTY",
                "api_concurrencies": 5,
        ...
        # PlatformConfig(
        #     **{
        #         "platform_name": "custom_openai",
        #         "platform_type": "custom_openai",
        #         "api_base_url": "http://rayservice-autoscaler-serve-svc.kuberay-system.svc:8000",
        #         "api_key": "EMPTY",
        #         "api_concurrencies": 50,
        #         "llm_models": [
        #             "qwen-1.5-0.5b-chat",
        #             "qwen-1.5-4b-chat",
        #             "qwen-1.5-7b-chat",
...
class ToolSettings(BaseFileSettings):
    ...
    model_config = SettingsConfigDict(
        yaml_file=AITUTOR_ROOT / "tool_settings.yaml",
        json_file=AITUTOR_ROOT / "tool_settings.json",
        extra="allow",
    )
...
class PromptSettings(BaseFileSettings):
    ...
    model_config = SettingsConfigDict(
        yaml_file=AITUTOR_ROOT / "prompt_settings.yaml",
        json_file=AITUTOR_ROOT / "prompt_settings.json",
        extra="allow",
    )
...
class SettingsContainer:
    ...
    def createl_all_templates(self):
        ...
        self.model_settings.create_template_file(
            sub_comments={
                "MODEL_PLATFORMS": {
                    "model_obj": PlatformConfig(),
                    "is_entire_comment": True,
                }
            },
            write_file=True,
        )
        self.tool_settings.create_template_file(
            write_file=True, file_format="yaml", model_obj=ToolSettings()
        )
