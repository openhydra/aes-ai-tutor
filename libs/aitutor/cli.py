...
from aitutor.init_database import create_tables, folder2db
from aitutor.init_database import main as kb_main
from aitutor.server.utils import get_default_embedding
from aitutor.settings import Settings
from aitutor.startup import main as startup_main
from aitutor.utils import build_logger

logger = build_logger()


@click.group(help="aitutor 命令行工具")
def main():
    ...
...
@click.option("-l", "--llm-model", help="指定默认 LLM 模型。默认为 qwen1.5-7b-chat")
@click.option(
    "-e", "--embed-model", help="指定默认 Embedding 模型。默认为 bge-large-zh-v1.5"
)
@click.option(
    "-r",
    "--recreate-kb",
...
def init(
    ...
    logger.success(f"开始初始化项目数据目录：{Settings.AITUTOR_ROOT}")
    Settings.basic_settings.make_dirs()
    shutil.copytree(
            bs.PACKAGE_ROOT / "data/nltk_data",
            Path(Settings.AITUTOR_ROOT) / "data/nltk_data",
            dirs_exist_ok=True,
        )
    logger.success("创建所有数据目录：成功。")
    if (
        bs.PACKAGE_ROOT / "data/knowledge_base/xedu"
        != Path(bs.KB_ROOT_PATH) / "xedu"
    ):
        shutil.copytree(
            bs.PACKAGE_ROOT / "data/knowledge_base/xedu",
            Path(bs.KB_ROOT_PATH) / "xedu",
    ...
    logger.success("复制 xedu 知识库文件：成功。")
    create_tables()
    ...
    logger.success(
        "请先检查确认 model_settings.yaml 里模型平台、LLM模型和Embed模型信息已经正确"
    )
    ...
    if recreate_kb:
        ...
        logger.success(
            "<green>所有初始化已完成，执行 aitutor start -a 启动服务。</green>"
        )
    else:
        logger.success(
            "执行 aitutor kb -r 初始化知识库，然后 aitutor start -a 启动服务。"
        )
