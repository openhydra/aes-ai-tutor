...
class KnowledgeFileModel(Base):
    ...
    kb_id = Column(String(64), ForeignKey("knowledge_base.id"), comment="知识库ID")
    ...
    knowledge_base = relationship(
        "KnowledgeBaseModel", back_populates="knowledge_files"
    )

    def __repr__(self):
        return f"<KnowledgeFile(id='{self.id}', kb_id='{self.kb_id}', file_name='{self.file_name}', file_ext='{self.file_ext}', kb_name='{self.kb_name}', document_loader_name='{self.document_loader_name}', text_splitter_name='{self.text_splitter_name}', file_version='{self.file_version}', create_time='{self.create_time}')>"
...
class FileDocModel(Base):
    ...
    knowledge_base = relationship("KnowledgeBaseModel", back_populates="file_docs")

    def __repr__(self):
        return f"<FileDoc(id='{self.id}', kb_id='{self.kb_id}', kb_name='{self.kb_name}', file_name='{self.file_name}', doc_id='{self.doc_id}', metadata='{self.meta_data}')>"
