...
from aitutor.server.api_server.api_schemas import OpenAIChatInput, OpenAIChatOutput
from aitutor.server.chat.file_chat import upload_temp_docs
from aitutor.server.chat.kb_chat import chat_with_kb
from aitutor.server.knowledge_base.kb_api import (
    create_kb,
    delete_kb,
    get_kb_info,
    list_kbs,
...
from aitutor.server.knowledge_base.kb_doc_api import (
    delete_docs,
    download_doc,
    list_files,
...
from aitutor.server.knowledge_base.kb_summary_api import (
    recreate_summary_vector_store,
    summary_doc_ids_to_vector_store,
    summary_file_to_vector_store,
)
from aitutor.server.utils import BaseResponse, ListResponse
...
kb_router.get(
    "/list_knowledge_bases/users/{user_id}",
    response_model=ListResponse,
    summary="获取用户的知识库列表",
)(list_kbs)

kb_router.get(
    "/list_public_knowledge_base",
    response_model=ListResponse,
    summary="获取公开知识库列表",
)(list_public_kbs)

kb_router.get(
    "/get_knowledge_base_detail/{kb_id}",
...
kb_router.delete(
    "/delete_knowledge_base/{kb_id}", response_model=BaseResponse, summary="删除知识库"
)(delete_kb)

kb_router.get(
    "/list_files/{kb_id}", response_model=ListResponse, summary="获取知识库内的文件列表"
)(list_files)
...
kb_router.post("/search_temp_docs", summary="检索临时知识库")(search_temp_docs)
