...
from aitutor.server.callback_handler.agent_callback_handler import AgentStatus  # noaq
from aitutor.server.pydantic_v2 import AnyUrl, BaseModel, Field
from aitutor.server.utils import MsgType, get_default_llm
...
class ConversationInput(BaseModel):
    id: Optional[str] = None
    user_id: Optional[str] = None
    name: str = Field(default="llm_chat", example="llm_chat")
    chat_type: Optional[str] = None
    temp_kb_id: Optional[str] = None
...
class HttpCustomeResponseMessage(BaseModel):
    custome_error_code: int = Field(default=0, example=0)
    message: str
