...
from aitutor.server.pydantic_v1 import Field
from aitutor.server.utils import get_tool_config
from aitutor.settings import Settings

from .tools_registry import BaseToolOutput, format_context, regist_tool
...
def search_engine(query: str, top_k: int = 0, engine_name: str = "", config: dict = {}):
    ...
    docs = [
        x
        for x in search_result2docs(results)
        if x.page_content and x.page_content.strip()
    ]
