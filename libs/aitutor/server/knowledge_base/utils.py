...
from aitutor.server.file_rag.text_splitter import (
    zh_title_enhance as func_zh_title_enhance,
)
from aitutor.server.utils import run_in_process_pool, run_in_thread_pool
...
def get_kb_path(user_id: str, knowledge_base_name: str):
    return os.path.join(Settings.basic_settings.KB_ROOT_PATH, user_id, knowledge_base_name)


def get_doc_path(user_id: str, knowledge_base_name: str):
    return os.path.join(get_kb_path(user_id, knowledge_base_name), "content")


def get_vs_path(user_id: str, knowledge_base_name: str, vector_name: str):
    return os.path.join(get_kb_path(user_id, knowledge_base_name), "vector_store", vector_name)


def get_file_path(user_id: str, knowledge_base_name: str, doc_name: str):
    doc_path = Path(get_doc_path(user_id, knowledge_base_name)).resolve()
...
def list_files_from_folder(user_id: str, kb_name: str):
    ...
    try:
        if loader_name in [
            ...
            document_loaders_module = importlib.import_module(
                "aitutor.server.file_rag.document_loaders"
            )
        else:
            document_loaders_module = importlib.import_module(
...
def make_text_splitter(splitter_name, chunk_size, chunk_overlap):
    ...
    try:
        if (
            ...
            headers_to_split_on = Settings.kb_settings.text_splitter_dict[
                splitter_name
            ]["headers_to_split_on"]
            ...
            try:  # 优先使用用户自定义的text_splitter
                text_splitter_module = importlib.import_module(
                    "aitutor.server.file_rag.text_splitter"
                )
            ...
            if (
                Settings.kb_settings.text_splitter_dict[splitter_name]["source"]
                == "tiktoken"
            ):  # 从tiktoken加载
                try:
                    text_splitter = TextSplitter.from_tiktoken_encoder(
                        encoding_name=Settings.kb_settings.text_splitter_dict[
                            splitter_name
                ...
                if (
                    Settings.kb_settings.text_splitter_dict[splitter_name][
                        "tokenizer_name_or_path"
                    ]
                    == "gpt2"
                ):
