...
from fastapi import Body, File, Path, Query, UploadFile
from fastapi.responses import FileResponse
from langchain.docstore.document import Document
from sse_starlette import EventSourceResponse

from aitutor.server.db.repository.knowledge_file_repository import get_file_detail
from aitutor.server.knowledge_base.kb_cache.faiss_cache import memo_faiss_pool
from aitutor.server.knowledge_base.kb_service.base import (
    KBServiceFactory,
    get_kb_file_details,
)
from aitutor.server.knowledge_base.model.kb_document_model import DocumentWithVSId
from aitutor.server.knowledge_base.utils import (
    KnowledgeFile,
...
def search_temp_docs(
    ...
    score_threshold: int | float = Body(..., description="分数阈值", examples=[0.8]),
) -> List[Dict]:
...
def search_docs(
    ...
    if kb is not None:
        if query:
            ...
            data = [
                DocumentWithVSId(**{"id": x.metadata.get("id"), **x.dict()})
                for x in docs
            ]
...
def list_files(
    ...
    if kb is None:
        return ListResponse(
            code=404, msg=f"未找到知识库 id: {kb_id}", data=[]
        )
    else:
        all_docs = get_kb_file_details(kb_id)
        return ListResponse(data=all_docs)


def _save_files_in_thread(
    user_id: str, files: List[UploadFile], knowledge_base_name: str, override: bool
):
    """
    ...
    def save_file(file: UploadFile, knowledge_base_name: str, override: bool) -> dict:
        ...
        try:
            ...
            if file_byte > 10 * 1024 * 1024:
                ...
                return dict(code=500, msg=msg, data={"knowledge_base_name": knowledge_base_name, "file_name": file.filename})
            filename = file.filename
            file_path = get_file_path(
                user_id=user_id, knowledge_base_name=knowledge_base_name, doc_name=filename
            )
...
def upload_docs(
    ...
    for result in _save_files_in_thread(
        kb.user_id, files, knowledge_base_name=kb_name, override=override
    ):
    ...
    if to_vector_store:
        ...
        if result.data is not None:
            failed_files.update(result.data.get("failed_files", {}))
        if not not_refresh_vs_cache:
            kb.save_vector_store()

    return BaseResponse(
        code=200, msg="文件上传与向量化完成", data={"succeeded_files": succeeded_files, "failed_files": failed_files}
    )
    ...
    for file_name in file_names:
        ...
        try:
            kb_file = KnowledgeFile(
                user_id=kb.user_id, filename=file_name, knowledge_base_name=knowledge_base_name
            )
...
def update_info(
    ...
    is_private: bool = Body(True, description="是否私有知识库"),
):

    kb = KBServiceFactory.get_service_by_id(kb_id)
    if kb is None:
        return BaseResponse(code=404, msg=f"未找到知识库 id:{kb_id}")
    kb.update_info(kb_info, is_private)
    ...
    for file_name in file_names:
        ...
        if file_name not in docs:
            try:
                kb_files.append(
                    KnowledgeFile(
                        user_id=kb.user_id, filename=file_name, knowledge_base_name=kb_name
                    )
                )
            except Exception as e:
...
def download_doc(
    kb_id: str = Query(..., examples=["fdd34328-48b4-4682-b69c-52bdf8950697"]),
    kb_name: str = Query(
        ..., description="知识库名称", examples=["samples"]
    ),
...
def recreate_vector_store(
    ...
    by default, get_service_by_id only return knowledge base in the info.db and having document files in it.
    ...
    def output():
        try:
            ...
            if not kb.exists() and not allow_empty_kb:
                yield {"code": 404, "msg": f"未找到知识库 id:‘{kb_id}’"}
            else:
                ...
                if not ok:
                    ...
                    if kb.exists():
                        kb.clear_vs()
                    kb.create_kb()
                    files = list_files_from_folder(kb.user_id, kb.kb_name)
                    kb_files = [(file, kb.kb_name) for file in files]
                    i = 0
                    for status, result in files2docs_in_thread(
                        kb_files,
                        chunk_size=chunk_size,
                        ...
                        if status:
                            kb.kb_name, file_name, docs = result
                            kb_file = KnowledgeFile(
                                user_id=kb.user_id, filename=file_name, knowledge_base_name=kb.kb_name
                            )
                            ...
                            kb.kb_name, file_name, error = result
                            msg = f"添加文件‘{file_name}’到知识库‘{kb.kb_name}’时出错：{error}。已跳过。"
                            logger.error(msg)
                            yield json.dumps(
                                {
                                    "code": 500,
