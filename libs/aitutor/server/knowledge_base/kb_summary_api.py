...
from aitutor.server.knowledge_base.kb_summary.base import KBSummaryService
from aitutor.server.knowledge_base.kb_summary.summary_chunk import SummaryAdapter
from aitutor.server.knowledge_base.model.kb_document_model import DocumentWithVSId
from aitutor.server.knowledge_base.utils import list_files_from_folder
...
def recreate_summary_vector_store(
    user_id: str = Body(..., examples=["user1"]),
            ...
            if not kb.exists() and not allow_empty_kb:
                ...
                if not ok:
                    ...
                    summary = SummaryAdapter.form_summary(
                        ...
                        overlap_size=Settings.kb_settings.OVERLAP_SIZE,
                    )
                    files = list_files_from_folder(user_id, knowledge_base_name)
                    ...
                    for i, file_name in enumerate(files):
                        ...
                        if status_kb_summary:
                            logger.info(
                                f"({i + 1} / {len(files)}): {file_name} 总结完成"
                            )
                            yield json.dumps(
                                {
                                    "code": 200,
