...
from aitutor.server.db.models.knowledge_base_model import KnowledgeBaseSchema
...
from aitutor.server.db.repository.knowledge_file_repository import (
    add_file_to_db,
    count_files_from_db,
    delete_file_from_db,
...
class KBService(ABC):
    ...
    def __repr__(self) -> str:
        return f"{self.user_id} @{self.kb_name} @ {self.embed_model}"
    ...
    def create_kb(self):
        ...
        status = add_kb_to_db(
            self.user_id, self.kb_id, self.kb_name, self.kb_info, self.vs_type(), self.embed_model, self.is_private
        )
    ...
    def drop_kb(self):
        ...
        """
        self.do_drop_kb()
        status = delete_kb_from_db(self.kb_id)
        return status
    ...
    def update_info(self, kb_info: str, is_private: bool):
        """
        更新知识库介绍
        """
        self.kb_info = kb_info
        self.is_private = is_private
        status = update_kb_info(
            self.kb_id, self.kb_info, self.is_private
        )
        return status
    ...
    def exist_doc(self, file_name: str):
        return file_exists_in_db(
            KnowledgeFile(knowledge_base_name=self.kb_name, user_id=self.user_id, filename=file_name)
        )
    ...
    def exists(self, kb_id: str = None):
        kb_id = kb_id or self.kb_id
        return kb_exists(kb_id)
...
class KBServiceFactory:
    ...
    def get_service(
        ...
        if SupportedVSType.FAISS == vector_store_type:
            from aitutor.server.knowledge_base.kb_service.faiss_kb_service import (
                FaissKBService,
            )

            return FaissKBService(**params)
        elif SupportedVSType.MILVUS == vector_store_type:
            from aitutor.server.knowledge_base.kb_service.milvus_kb_service import (
                MilvusKBService,
            )
            ...
            from aitutor.server.knowledge_base.kb_service.default_kb_service import (
                DefaultKBService,
            )
    ...
    @staticmethod
    def get_service_by_id(kb_id: str) -> KBService:

        user_id, kb_name, vs_type, embed_model = load_kb_from_db(kb_id)
        if kb_name is None:  # kb not in db, just return None
            return None
        return KBServiceFactory.get_service(user_id, kb_id, kb_name, vs_type, embed_model)
...
def get_kb_file_details(kb_id: str) -> List[Dict]:
    kb = KBServiceFactory.get_service_by_id(kb_id)
    if kb is None:
        return []

    files_in_folder = list_files_from_folder(kb.user_id, kb.kb_name)
    ...
    for doc in files_in_db:
        doc_detail = get_file_detail(kb.kb_name, doc)
        if doc_detail:
