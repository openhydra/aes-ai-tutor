...
from aitutor.server.file_rag.utils import get_Retriever
from aitutor.server.knowledge_base.kb_cache.faiss_cache import (
    ThreadSafeFaiss,
    kb_faiss_pool,
)
from aitutor.server.knowledge_base.kb_service.base import KBService, SupportedVSType
from aitutor.server.knowledge_base.utils import KnowledgeFile, get_kb_path, get_vs_path
...
class FaissKBService(KBService):
    ...
    def get_vs_path(self):
        return get_vs_path(self.user_id, self.kb_name, self.vector_name)

    def get_kb_path(self):
        return get_kb_path(self.user_id, self.kb_name)
    ...
    def do_search(
        ...
        query: str,
        top_k: int,
        score_threshold: int | float = Settings.kb_settings.SCORE_THRESHOLD,
