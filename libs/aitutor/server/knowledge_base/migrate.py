...
from aitutor.server.db.base import Base, engine_with_db
...
from aitutor.server.db.repository.knowledge_metadata_repository import (
    add_summary_to_db,
)
from aitutor.server.db.session import session_scope
...
from aitutor.server.utils import get_default_embedding
...
def reset_tables():
    Base.metadata.drop_all(bind=engine_with_db)
    create_tables()
...
def folder2db(
    ...
    vs_type: Literal["faiss", "milvus"] = Settings.kb_settings.DEFAULT_VS_TYPE,
    ...
    for kb_name in kb_names:
        ...
        if mode == "recreate_vs":
            kb.clear_vs()
            kb.create_kb()
            kb_files = file_to_kbfile(kb_name, list_files_from_folder(user_id, kb_name))
        ...
        # 对比本地目录与数据库中的文件列表，进行增量向量化
            ...
            folder_files = list_files_from_folder(user_id, kb_name)
...
def prune_db_docs(kb_ids: List[str]):
    """
    ...
    for kb_id in kb_ids:
        ...
        if kb is not None:
            ...
            kb_files = file_to_kbfile(kb.kb_name, files)
            for kb_file in kb_files:
                kb.delete_doc(kb_file, not_refresh_vs_cache=True)
                print(f"success to delete docs for file: {kb.kb_name}/{kb_file.filename}")
            kb.save_vector_store()


def prune_folder_files(kb_ids: List[str]):
    """
    ...
    for kb_id in kb_ids:
        ...
        if kb is not None:
            files_in_db = kb.list_files()
            files_in_folder = list_files_from_folder(kb.user_id, kb_id)
            files = list(set(files_in_folder) - set(files_in_db))
            for file in files:
                os.remove(get_file_path(kb.kb_name, file))
                print(f"success to delete file: {kb.kb_name}/{file}")
